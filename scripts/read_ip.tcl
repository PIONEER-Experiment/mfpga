...
# trigger number FIFO
if {[file exists $ROOT/ip/trig_num_axis_data_fifo/trig_num_axis_data_fifo.xci
]} {
	read_ip $ROOT/ip/trig_num_axis_data_fifo/trig_num_axis_data_fifo.xci
} else {
	create_ip -name axis_data_fifo -vendor xilinx.com -library ip -module_name trig_num_axis_data_fifo -dir $ROOT/ip
	set_property -dict [list CONFIG.TDATA_NUM_BYTES {3} CONFIG.FIFO_DEPTH {512}] [get_ips trig_num_axis_data_fifo]
	generate_target all [get_files $ROOT/ip/trig_num_axis_data_fifo/trig_num_axis_data_fifo.xci]
	synth_ip [get_ips trig_num_axis_data_fifo]
}
...
if {[file exists $ROOT/ip/axis_switch_rx/axis_switch_rx.xci
	...
	set_property -dict [list CONFIG.NUM_SI {5} CONFIG.NUM_MI {1} CONFIG.TDATA_NUM_BYTES {4} CONFIG.HAS_TLAST {1} CONFIG.ARB_ON_MAX_XFERS {1024} CONFIG.ARB_ON_NUM_CYCLES {1} CONFIG.ARB_ALGORITHM {1}] [get_ips axis_switch_rx]
...
if {[file exists $ROOT/ip/axis_switch_tx/axis_switch_tx.xci
	...
	set_property -dict [list CONFIG.NUM_SI {1} CONFIG.NUM_MI {5} CONFIG.TDATA_NUM_BYTES {4} CONFIG.HAS_TLAST {1} CONFIG.TDEST_WIDTH {4}] [get_ips axis_switch_tx]
